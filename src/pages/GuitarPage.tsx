        ...
        <div className="flex items-center gap-4 text-[11px] font-mono tracking-widest text-muted-foreground/60">
          ...
          <div className="flex flex-1 items-center gap-2">
            {activeStages.map((stage, index) => {
              ...
              return <span key={stageIndex} style={{ clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }} className={`h-[4px] flex-1 rounded-full transition-colors duration-500 ${fill}`} />;
            })}
          </div>
        </div>
      </div>
